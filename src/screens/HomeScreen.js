...
const App = () => {
  ...
  const speechResultsHandler = e => {
    ...
    setResult(prevResult => {
      // Use the previous result to update the state
      const newResult = text;
      fetchResponse(newResult.trim());
      return newResult;
    });
  };
  ...
  const fetchResponse = newResult => {
    console.log('result', newResult);
    if (newResult.length > 0) {
      setMessages(prevMessages => {
        const newMessages = [
          ...prevMessages,
          {role: 'user', content: newResult},
        ];
        updateScrollView();
        setLoading(true);
        apiCall(newResult, newMessages).then(res => {
